...
function applyTheme(theme) {
  ...
  // Show the icon for the next theme to apply
...
async function loadQueue() {
  try {
    ...
    if (queueStatus) {
      queueStatus.textContent = queueData.length === 0 ? 'No patients in queue' : `${queueData.length} waiting`;
    }
    offlineBanner.hidden = true;
  } catch (err) {
    queueList.innerHTML = '';
    queueEmpty.hidden = false;
    if (queueStatus) queueStatus.textContent = '';
    offlineBanner.hidden = false;
  }
}
